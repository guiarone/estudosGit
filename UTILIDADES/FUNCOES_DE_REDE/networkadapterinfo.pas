...
type
  ...
  function Lista_Placas_de_Rede(var MACs, IPs, Mascaras: TStringList): boolean;
...
var
  ...
  k           : integer;
  ErrCode     : longint;
  i_pTableSize: longint;
  pIPTable    : P_IpAddrTable;
  sAddr       : in_addr;
  sMask       : in_addr;
begin
  retorno:= true;
//limpa para garantir mesmo indice
  MACs.Clear;
  IPs.Clear;
  Mascaras.Clear;
//inicializa
  pIPTable:= nil;
  try
    //le para descobrir tamanho
    i_pTableSize := 0;
    GetIpAddrTable(pIpTable, i_pTableSize, True);
    if (i_pTableSize < SizeOf(MIB_IPADDRROW) + Sizeof(longint)) then
      retorno := False
    else
    begin
      //aloca memoria
      GetMem(pIpTable, i_pTableSize);
      ErrCode := GetIpAddrTable(pIpTable, i_pTableSize, True);
      //
      ...
      begin
        ...
        begin
          ...
          //
          auxIP  := Format('%s', [inet_ntoa(sAddr)]);
          auxMASC:= Format('%s', [inet_ntoa(sMask)]);
          //
          MACs.Add('00-00-00-00-00-00');
          IPs.Add(auxIP);
          Mascaras.Add(auxMASC);
        end;
      end;
    end;
  except
...
function Get_EthernetAdapterDetail(var AdapterDataFound: TAdapterInfo): boolean;
var
  pIfTable: P_IfTable;//  ^_IfTable;
  pIpTable: P_IpAddrTable;// ^_IpAddrTable;
  ...
  try
    ...
    for i := 1 to pIfTable^.nRows do
      try
        ...
        //begin
        m := i - 1;
        AdapterDataFound[m].dwIndex := (pIfTable^.ifRow[i].dwIndex);//4;//
        AdapterDataFound[m].dwType := (pIfTable^.ifRow[i].dwType);
        AdapterDataFound[m].dwIndex := (pIfTable^.ifRow[i].dwIndex);
        AdapterDataFound[m].sIpAddress := IPAddresses.Values[Format('0x%8.8x', [(pIfTable^.ifRow[i].dwIndex)])];
        AdapterDataFound[m].sIpMask := IPMasks.Values[Format('0x%8.8x', [(pIfTable^.ifRow[i].dwIndex)])];
        ...
        tmp := '';
        //if pIfTable^.ifRow[i].dwPhysAddrLen < 20 then//proteção guiarone
