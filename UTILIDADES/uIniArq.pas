...
begin
  inherited Create;
  FnomeArq:= StrPCopy( StrAlloc( Succ( Length( aNomeArq ) ) ), aNomeArq );
end;
...
var
  oAnt: Char;
begin
  oAnt:= FormatSettings.DecimalSeparator; // guarda indicador de casas decimais
  FormatSettings.DecimalSeparator:= ',';  // muda indicador de casa decimal para vírgula
  GravaString( Secao, Item, FloatToStr( Conteudo ) );
  FormatSettings.DecimalSeparator:= oAnt; // restaura indicador de casas decimais
end;
  ...
  try
    ...
    try
      ...
      ShowMessage( 'O item "' + UpperCase( Item ) +
                   '" da seção [' + UpperCase( Secao ) +
                   '] do arquivo ' + getNomeArq + ' não é válido!' );
...
function TIniArq.getNomeArq: string;
begin
  Result:= ExtractFileName( StrPas( FnomeArq ) );
end;
